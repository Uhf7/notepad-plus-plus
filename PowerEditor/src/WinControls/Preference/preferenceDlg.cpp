...
INT_PTR CALLBACK SettingsDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM)
{
	...
	switch (message) 
	{
		case WM_INITDIALOG :
		{
			...
			::SendDlgItemMessage(_hSelf, IDC_CHECK_DIRECTWRITE_ENABLE, BM_SETCHECK, nppGUI._useDirectWrite, 0);
		...
		case WM_COMMAND : 
		{
			...
			switch (wParam)
			{
				...
				{
					nppGUI._useDirectWrite = isCheckedOrNot(IDC_CHECK_DIRECTWRITE_ENABLE);
					return TRUE;
				}
